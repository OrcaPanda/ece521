...
with np.load("notMNIST.npz") as data :
    ...
    l_d = tf.scalar_mul(0.5, tf.reduce_mean(tf.pow(tf.subtract(y_pred, y_target), 2)))
    ...
    for i in range(epoch):
        ...
        # Computing accuracy
        predictions = sess.run(y_pred, feed_dict={x: compData, y_target: compTarget})
        correct = 0.0
        count = 0
    ...
    #pickle.dump(accuracy_array_train, open("q1_A_training_accuracy","wb"))
    ...
    plt.ylabel('Accuracy')
    plt.xlabel('Epochs')
    plt.title('Adam Linear notMNIST Accuracy lr-0.0001')
    ...
    plt.xlabel('Epochs')
    plt.title('Adam Linear notMNIST Cross Entropy Loss lr-0.0001')
    plt.legend(bbox_to_anchor=(.7, .8), loc=2, borderaxespad=0.)
    plt.show()
